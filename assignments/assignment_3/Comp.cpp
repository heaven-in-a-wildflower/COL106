...
void Comp::createAccount(std::string id, int count) {
    ...
    table_size++;
    load_factor = double(table_size) / bucket_array_size; // Calculate load factor
    if (load_factor > 0.7)
        resize(true);
}
...
int Comp::hash(std::string id)  {
    ...
    return hash_value;
    // std::string alphabets = id.substr(0, 4);
    // std::string digits1 = id.substr(4, 7);
    // char underscore = id[11];
    // std::string digits2 = id.substr(12, 10);

    // // Hash the relevant parts using a combination of their ASCII values
    // long long hashValue = 0;

    // // Hash the first 4 alphabets
    // for (char c : alphabets) {
    //     hashValue = hashValue * 31 + (c - 'A' + 1);
    // }

    // // Hash the first 7 digits
    // for (char c : digits1) {
    //     hashValue = hashValue * 31 + (c - '0' + 1);
    // }

    // // Include the underscore (assuming its ASCII value is fixed)
    // hashValue = hashValue * 31 + (underscore - '0' + 1);
    ...
    // return hashValue % bucket_array_size;
}

void Comp::resize(bool flag)
{
    if (flag == true)
        ptr++;
    else
